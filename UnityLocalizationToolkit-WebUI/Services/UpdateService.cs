    ...
    ILogger<UpdateService> logger)
{
    ...
    private const string GitHubRepo = "UnityLocalizationToolkit-WebUI";
        ...
        CancellationToken ct = default, bool bypassThrottle = false)
    {
        ...
        try
        {
            ...
            if (_lastCheckResult.UpdateAvailable)
            {
                ...
                trayService.ShowNotification("UnityLocalizationToolkit", $"发现新版本 v{remoteVersion} 可用");
            }
            else
            {
                _status = new UpdateStatusInfo { State = UpdateState.None };
    ...
    public async Task DownloadUpdateAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            trayService.ShowNotification("UnityLocalizationToolkit", "更新已就绪，请在设置中重启更新");
        }
        ...
        catch (Exception ex)
        {
            ...
            await BroadcastStatus();
            trayService.ShowNotification("UnityLocalizationToolkit", "下载更新失败，请重试");
            throw;
        }
        finally
        {
    ...
    public async Task<string> ApplyUpdateAsync(CancellationToken ct = default)
    {
        ...
        var exeName = Path.GetFileName(Environment.ProcessPath ?? "UnityLocalizationToolkit-WebUI.exe");
        var deleteListPath = Path.Combine(stagingDir, "delete-list.txt");
