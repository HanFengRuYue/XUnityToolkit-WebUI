...
export const useInstallStore = defineStore('install', () => {
    ...
    const operationType = ref('install');
    const healthReport = ref(null);
    const skippedSteps = ref(new Set());
    let connection = null;
    async function connectHub(gameId) {
        if (connection) {
            try {
        ...
        connection.on('healthReportReady', (report) => {
            healthReport.value = report;
        });
        connection.onreconnected(async () => {
            if (activeGameId.value) {
                try {
                    await connection?.invoke('JoinGameGroup', activeGameId.value);
                }
                catch { /* ignore */ }
            }
        });
     ...
     */
    async function resumeInstall(gameId, backendStatus) {
    ...
    function computeSkippedSteps(options) {
        if (!options)
            return new Set();
        const steps = new Set();
        if (!options.autoInstallTmpFont)
            steps.add('InstallingTmpFont');
        if (!options.autoDeployAiEndpoint)
            steps.add('InstallingAiTranslation');
        if (!options.autoGenerateConfig)
            steps.add('GeneratingConfig');
        if (!options.autoApplyOptimalConfig)
            steps.add('ApplyingConfig');
        if (!options.autoExtractAssets)
            steps.add('ExtractingAssets');
        if (!options.autoVerifyHealth)
            steps.add('VerifyingHealth');
        return steps;
    }
    async function startInstall(gameId, config, options) {
    ...
    async function startUninstall(gameId) {
