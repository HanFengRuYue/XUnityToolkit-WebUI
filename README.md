...
- 上传 TTF / OTF 后说：`把这个字体作为运行时 TMP 回退字体。`（会要求确认，不会自动改写游戏资源。）
...
- 新安装默认部署 `XUnityToolkit.RuntimeFontLoader`：优先直接从思源黑体 TTF 创建动态 TMP 字体，并作为全局回退保留原游戏字体风格；也可切换为“全部 TMP 文本替换”。
- 如果游戏的 TMP 不提供文件路径创建 API，内置思源黑体会自动使用匹配 Unity 版本的兼容 TMP AssetBundle；自定义字体不会静默换成内置字体，而会显示错误并提供“生成兼容 TMP 资产”入口。
...
- 先在游戏配置的 **字体与排版** 中安装运行时 TMP 字体；默认“全局回退”通常不会改变原有拉丁字体风格。
- 若状态显示直接 TTF API 不可用，内置思源黑体会自动回退到兼容资产；自定义字体则需要点击 **生成兼容 TMP 资产**。
- 只有需要直接改写游戏内 TMP / Legacy `Font` 资源时，才进入 **字体替换** 页面扫描并替换资源。
