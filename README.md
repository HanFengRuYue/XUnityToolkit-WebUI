...
5. 从左侧打开 **工具箱智能体**，可以用自然语言调用工具箱功能；选择游戏并上传 TTF/OTF 后，可自动生成 TMP 字体、替换支持的游戏字体并写入 fallback 配置。
...
- **可操作的工具箱智能体**：左侧导航打开悬浮对话窗；云端 AI 可在受控工具目录内调用游戏管理、安装、配置、术语、译文、字体、插件、日志、更新和本地模型管理等现有功能，并接收字体、插件、配置或图片附件。
...
- **智能体安全边界**：智能诊断、自动修复与工具箱智能体仅支持云端 AI；本地 AI 模式会明确提示不支持。DLL 只读取程序集元数据与引用，不上传二进制、不执行插件；附件二进制留在本机，只有完成任务所需的脱敏文本、元数据与工具结果会发送到当前云端端点。文本会限制在游戏目录白名单内并过滤重解析点、密钥、Token、用户名与绝对路径。模型不能运行系统命令或直接获得任意文件权限，高影响 API 需要用户确认；自动修复只允许可验证的组件恢复、端点路由、INI/CFG 单键修改和第三方插件禁用。多阶段模型调用可能产生当前端点的 API 费用。
...
- 本地模式对显卡、显存和磁盘空间要求更高。
- 插件智能诊断、全自动修复和左侧工具箱智能体需要云端 AI；切换到本地模式时这些功能会显示“不支持运行”，本地模型仍可用于普通翻译。
