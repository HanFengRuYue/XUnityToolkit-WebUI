...
一键安装 BepInEx 与 XUnity.AutoTranslator，集成云端大模型、本地 llama.cpp、实时 AI 翻译、字体替换、插件自动修复与可操作工具箱的 AI 智能体。
...
5. 从左侧打开 **工具箱智能体**，选择已配置的云端端点并用自然语言调用工具箱功能；历史对话会保存在本机，选择游戏并上传 TTF/OTF 后还可自动生成 TMP 字体、替换支持的游戏字体并写入 fallback 配置。
...
- **可操作的工具箱智能体**：左侧导航打开悬浮对话窗；可自动或手动选择已配置的云端端点，在受控工具目录内调用游戏管理、安装、配置、术语、译文、字体、插件、日志、更新和本地模型管理等现有功能，并接收字体、插件、配置或图片附件。最近 100 个对话保存在本机，可随时恢复或删除。
...
- **插件智能诊断与自动修复**：打开游戏详情或 BepInEx 日志页时只刷新本地客观事实，不调用模型。手动点击“AI 智能诊断”后，第一阶段由云端 AI 从脱敏资料清单中选择关键日志/配置，第二阶段依据带行号的证据生成问题与置信度；“AI 全自动修复”会继续生成受限工具计划，备份并执行后再复检。“启动并智能诊断”还会先取得本次游戏启动日志和工具箱 ping。
- **智能体安全边界**：智能诊断、自动修复与工具箱智能体仅使用云端 AI；工具箱智能体可独立于普通翻译的本地/云端模式使用窗口中选择的云端端点。DLL 只读取程序集元数据与引用，不上传二进制、不执行插件；附件二进制留在本机，只有完成任务所需的脱敏文本、元数据与工具结果会发送到所选云端端点。文本会限制在游戏目录白名单内并过滤重解析点、密钥、Token、用户名与绝对路径。模型不能运行系统命令或直接获得任意文件权限，高影响 API 需要用户确认；自动修复只允许可验证的组件恢复、端点路由、INI/CFG 单键修改和第三方插件禁用。多阶段模型调用可能产生所选端点的 API 费用。
...
- 本地模式对显卡、显存和磁盘空间要求更高。
- 插件智能诊断和全自动修复要求普通翻译处于云端模式；工具箱智能体则可在窗口中独立选择已配置的云端端点，因此普通翻译使用本地模型时仍可运行，但不会改用 llama-server。
