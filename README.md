...
| 版本 | ZIP 便携版 | MSI 安装包 |
|------|-----------|-----------|
| **Full（完整版）** | [下载](https://github.com/HanFengRuYue/XUnityToolkit-WebUI/releases/download/v4.4/XUnityToolkit-WebUI-v4.4-win-x64.zip) | [下载](https://github.com/HanFengRuYue/XUnityToolkit-WebUI/releases/download/v4.4/XUnityToolkit-WebUI-v4.4-win-x64.msi) |
| **No-LLAMA** | [下载](https://github.com/HanFengRuYue/XUnityToolkit-WebUI/releases/download/v4.4/XUnityToolkit-WebUI-v4.4-win-x64-no-llama.zip) | [下载](https://github.com/HanFengRuYue/XUnityToolkit-WebUI/releases/download/v4.4/XUnityToolkit-WebUI-v4.4-win-x64-no-llama.msi) |
| **Lite（精简版）** | [下载](https://github.com/HanFengRuYue/XUnityToolkit-WebUI/releases/download/v4.4/XUnityToolkit-WebUI-v4.4-win-x64-lite.zip) | [下载](https://github.com/HanFengRuYue/XUnityToolkit-WebUI/releases/download/v4.4/XUnityToolkit-WebUI-v4.4-win-x64-lite.msi) |
<!-- DOWNLOAD_LINKS_END -->
...
### 🤖 AI 翻译
接入 OpenAI / Claude / Gemini / DeepSeek / 通义千问 / 智谱 GLM / Kimi 等多家大模型 API，支持多提供商负载均衡，多阶段翻译管线 + 术语审查确保翻译质量
...
### 📖 统一术语管理
自定义翻译/禁翻术语，AI 自动提取专有名词，多阶段翻译管线（自然翻译→占位符→强制修正）确保术语准确
...
### ⚙️ 更多功能
配置可视化编辑 · 脚本标签清理 · 插件包导出/导入 · 在线更新 · 系统托盘 · 深色/浅色主题 · 镜像加速
...
| **本地大模型** | NVIDIA 显卡需要 CUDA 13+；AMD/Intel 显卡需支持 Vulkan；也可使用 CPU 推理 |
...
如果游戏翻译后出现方块字（「口口口」）或中文缺字，可能是游戏使用的 TextMesh Pro 字体，或依赖系统回退 / 字体名映射的旧版 Unity `Font` 不包含中文字符，需要进行字体替换：

1. 在游戏详情页进入 **「字体替换」** 功能
2. 点击 **「扫描」** — 工具会扫描游戏中的 TMP 字体资产，以及可识别的旧版 Unity `Font` 资源
3. 查看扫描结果，确认需要替换的字体
4. 选择替换方式：
   - **内置 TMP 字体**：使用预置的中文 TMP 字体（自动匹配 Unity 版本）
   - **自定义字体上传**：上传自己的 TMP 字体文件
5. 点击 **「替换」** 完成字体替换；对于“系统回退 / 名称映射”模式的旧版 `Font`，工具会将其转为内嵌动态字体再写回资源
...
1. 在游戏详情页进入 **「术语编辑器」** 页面
2. **添加翻译术语**：输入原文和对应的译文（如 `Dragonborn` → `龙裔`），类型选择「翻译」
3. **添加禁翻术语**：添加不需要翻译的词汇（如英文技能名 `Fireball`、物品代码 `ITEM_001`），类型选择「禁翻」
4. **设置术语属性**：每条术语可独立配置分类（角色/地点/物品等）、优先级、大小写敏感、精确匹配、正则表达式
5. **AI 自动提取**：在 AI 翻译设置中开启「自动提取术语」，AI 会在翻译过程中自动识别并添加专有名词
6. **编写游戏描述**：在 **「AI 描述」** 中编写游戏的背景介绍，帮助 AI 理解游戏世界观

**多阶段翻译管线**确保术语准确：

- **阶段 1（自然翻译）**：将术语以结构化提示词发送给 AI，不使用占位符，依赖 AI 理解
- **阶段 2（占位符替换）**：对阶段 1 未正确翻译的术语，使用 `{{G_x}}`/`{{DNT_x}}` 占位符强制替换
- **阶段 3（强制修正）**：对仍未通过术语审查的文本进行最终修正

> **提示**：术语按游戏独立维护，每个游戏最多支持 10000 条术语。
...
- 在「设置」页面检查更新，支持在线增量更新，更新器会自动处理文件替换和重启
- 可在设置中开启「接收预发布版本」以获取最新功能
- 也可以手动下载最新版本覆盖安装
...
- **多阶段翻译管线**：自然翻译 → 占位符替换 → 强制修正，逐阶段处理术语，最大化翻译质量
- **术语审查**：翻译完成后自动验证术语是否被正确应用，未通过的文本进入下一阶段重翻
- **脚本标签清理**：自动剥离游戏特定的指令代码（如 `tk,N,text`），避免干扰翻译；支持按游戏配置自定义规则和全局预设
...
- **预翻译缓存优化**：懒加载缓存，自动生成正则翻译文件（`r:` 标准正则和 `sr:` 分组正则），最大化缓存命中率
- **脚本标签清理**：自动剥离游戏特定指令代码，确保提取的文本干净可翻译
...
- **字体扫描**：自动扫描游戏的 `.assets` 和 AssetBundle 文件，找出所有 TextMesh Pro（TMP）字体资产，以及可识别的旧版 Unity `Font`
- **一键替换**：将游戏内的 TMP 字体替换为包含中文字符的版本；对依赖系统回退 / 字体名映射的旧版 `Font`，会原位转为内嵌动态字体并保留原有引用关系
...
- **统一术语系统**：翻译术语和禁翻术语合并管理，每条术语支持类型（翻译/禁翻）、分类（角色/地点/物品等）、优先级、大小写敏感、精确匹配、正则表达式等属性
- **优先级排序**：术语按优先级（高优先）和文本长度（长优先）排序，确保重要术语和长术语优先匹配，避免子串冲突
- **AI 自动提取**：开启后，AI 在翻译过程中自动识别和积累新的专有名词
- **术语审查**：翻译完成后自动检查术语是否被正确应用，未通过的文本进入下一翻译阶段
- **多阶段管线**：阶段 1（自然翻译）→ 阶段 2（占位符 `{{G_x}}`/`{{DNT_x}}` 替换）→ 阶段 3（强制修正），每个阶段只处理上一阶段未解决的文本
- **AI 游戏描述**：为每个游戏编写背景描述，帮助 AI 理解游戏的世界观和语境，提高翻译质量
- **术语上限**：单个游戏最多 10000 条术语
...
- **脚本标签清理**：按游戏配置规则，自动剥离游戏特定的指令代码（如 `tk,N,text`、`%%,N,text,#BTN`），内置全局版本化预设并支持自动更新
- **插件包导出/导入**：将游戏的翻译配置（框架 + 翻译端点 + 术语表 + 翻译缓存）打包为 ZIP，方便分享给其他玩家；导入时自动还原
- **在线更新**：基于 GitHub Releases 的差分增量更新，自动下载变更的组件；`Updater.exe`（AOT 编译，无运行时依赖）负责文件替换和重启；支持回滚；可选接收预发布版本
...
应用数据统一存储在 `%AppData%\XUnityToolkit\`（可通过 `AppData:Root` 配置项覆盖）。

| 路径 | 说明 |
|------|------|
| `library.json` | 游戏库数据 |
| `settings.json` | 应用设置（API 密钥经 DPAPI 加密） |
| `local-llm-settings.json` | 本地大模型设置 |
| `backups/` | 游戏备份清单（用于卸载还原） |
| `cache/` | 封面、图标、背景图缓存 |
| `glossaries/` | 按游戏维护的统一术语表（翻译 + 禁翻） |
| `script-tags/` | 按游戏维护的脚本标签清理规则 |
| `font-backups/` | 字体替换备份 |
| `custom-fonts/` | 用户上传的自定义字体 |
| `generated-fonts/` | SDF 字体生成器输出 |
| `logs/` | 应用日志 |
| `llama/` | llama.cpp 解压后的可执行文件 |
| `models/` | 下载的本地模型文件 |
| `update-staging/` | 在线更新暂存区 |
