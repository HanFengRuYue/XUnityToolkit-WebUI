...
- **VIDE Dialogues extraction:** `AssetExtractionService.TryExtractVideDialogue` detects VIDE dialogue JSON in TextAssets via `dID`+`playerDiags` keys; parses flat JSON, extracts dialogue text from `pd_{N}_com_{C}text` keys and character names from `pd_{N}_com_{C}charName` keys; filters control markers (`*` prefix, `RAND`, `ExtraData`); source tags `VIDE:{dialogueName}` and `VIDE:Character:{dialogueName}`; falls back to generic `ExtractTextLines` for non-VIDE TextAssets
- **VIDE SOQuotes_UW extraction:** `IsVideQuote` detects via `quotes`+`dialType` field presence; `ExtractVideQuotes` extracts all strings from `quotes` array; source tag `Quote:{assetName}`
- **VIDE SOTraitData_UW extraction:** `IsVideTrait` detects via `traitName`+`traitType` field presence; `ExtractVideTraits` extracts `traitName`, `reqsText`, `effectText`, `flavorText` directly; extracts `hoverText` only when it contains substantial non-template prose (strips `{NAME}`/`{REQS}`/`{EFFECT}`/`{FLAVOR}`/`{STATS}`/`{REPS}`/`{TP}`/`{ITEMS}`/`{CLOTHES}` placeholders, checks remaining text >5 chars); source tags `Trait:{assetName}` and `Trait:HoverText:{assetName}`
- **Adding game-specific extractors:** Pattern: detect via field signatures in `ExtractFromAssetsInstance` → dedicated extraction method with specific source tags → falls back to generic `CollectStrings`; for TextAsset JSON formats, detect in the TextAsset loop before `ExtractTextLines` fallback; add `[GeneratedRegex]` for key patterns; update CLAUDE.md architecture section
...
- **Pipeline Flow fork design:** Status Dashboard shows a horizontal forked pipeline: "已接收" → [LLM branch: 排队→翻译中→已完成→术语提取] | [TM branch: (精确·模糊·模式 inline node)→已命中]; the horizontal fork layout is the intentional design — do NOT replace with flat lane-card or vertical top-down layout; SVG overlay draws right-angle connection paths with animated particles (`offset-path`); all nodes wrapped in `NPopover` for hover tooltips; nodes use `flex: 1 1 0` to fill available width; term extraction is inline in LLM branch flow (after 已完成, NOT a separate `.branch-sub` below); TM hits displayed as single `.tm-node` with `.tm-chips-inline` (NOT separate small chips); LLM completed count is computed as `totalTranslated - (tmHits + tmFuzzyHits + tmPatternHits)`; TM branch uses accent-based `color-mix()` variants (NOT `--secondary`) to follow theme accent, real-time `TranslationStats` fields (not separately-fetched `TranslationMemoryStats`)
...
- **Term extraction services:** `GlossaryExtractionService` (reactive, hot-path, direct-to-term-table) and `TermExtractionService` (batch, pre-translation, staging candidates) share LLM extraction logic via `LlmResponseParser`, `EndpointSelector` (`SelectBestEndpoint` — no per-service endpoint override), `TermCategoryMapping` but are intentionally separate services — different lifecycles, outputs, and concurrency models; do NOT merge
- **GlossaryExtractionService retrigger guard:** When resetting `LastExtractionAt` on semaphore timeout, NEVER reset to 0 (causes immediate retrigger on every `POST /api/translate`); reset to `total - interval + 1` to allow retry after one interval
...
- **TranslationStats computed fields:** `DynamicPatternCount` and `ExtractedTermCount` are populated in `TranslateEndpoints.cs` (not `LlmTranslationService.GetStats()`) because `TermExtractionService` → `LlmTranslationService` creates a circular DI dependency; any new computed stats from services that depend on `LlmTranslationService` must follow the same endpoint-layer pattern
- **`RecentTranslation.EndpointName` for TM hits:** Set to `"翻译记忆"` in `PipelineComplete` when `perTextTranslationSource[i]` is not null; prevents empty endpoint name in recent translations list
- **`TranslationMemoryService.GetHitStats()` tuple order:** Returns `(ExactHits, PatternHits, FuzzyHits, Misses)` — destructure with matching names, NOT `(exact, fuzzy, pattern, misses)`
...
- **TermEntry deserialization invariant:** ALL code deserializing `List<TermEntry>` from JSON (including migration/import paths) MUST use `FileHelper.DataJsonOptions` — without the enum converter, `DoNotTranslate` type silently becomes `Translate` (default enum value 0)
...
- **Translation Memory display:** TM hit stats shown in Pipeline Fork (not a separate card); real-time data from `TranslationStats.TranslationMemoryHits/FuzzyHits/PatternHits` via SignalR (no separate TM stats API — `translationMemoryApi` was removed as dead code)
...
- **PreTranslationStatus fields:** Sync 2 places: `Models/AssetExtraction.cs`, `src/api/types.ts`; display in `AssetExtractionView.vue`; includes `CurrentRound`, `CurrentPhase`, `PhaseProgress`, `PhaseTotal`, `ExtractedTermCount`, `DynamicPatternCount`
...
- **Versioning:** `build.ps1` auto-generates `3.2.{YYYYMMDDHHmm}` (CI uses `3.2.` prefix) via `-p:InformationalVersion`; **must use `InformationalVersion` not `Version`** — `Version` sets `AssemblyVersion` (UInt16 max 65535) which overflows with timestamp
...
- **Bundled assets:** `bundled/{bepinex5,bepinex6,xunity,llama}/` — BepInEx/XUnity auto-detect latest versions via API; llama.cpp pinned to b8416 (update `$llamaTag` in build.ps1/build.yml to change); CUDA 12.4; copied post-publish
...
- **CI `Download-Asset` temp file:** Must use `.downloading` temp file + `Move-Item` pattern (matching `build.ps1` `Download-IfMissing`) to prevent interrupted downloads from being treated as cached on retry
