...
- **版本号：** `build.ps1` 通过 `-p:InformationalVersion` 自动生成 `4.2.{YYYYMMDDHHmm}`（CI 使用 `4.2.` 前缀）；**必须使用 `InformationalVersion` 而非 `Version`** — `Version` 设置 `AssemblyVersion`（UInt16 最大 65535），时间戳会溢出
