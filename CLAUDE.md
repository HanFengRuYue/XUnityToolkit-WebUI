...
- **Versioning:** `build.ps1` auto-generates `2.5.{YYYYMMDDHHmm}` (CI uses `2.5.` prefix) via `-p:InformationalVersion`; **must use `InformationalVersion` not `Version`** — `Version` sets `AssemblyVersion` (UInt16 max 65535) which overflows with timestamp
