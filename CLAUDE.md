...
- **Sidebar State:** `useSidebarStore` (Pinia + localStorage) manages collapse/width; `effectiveWidth` computed returns 64px when collapsed, custom width otherwise; mobile (≤768px) ignores collapse/resize
...
- **Pipeline Flow fork design:** Status Dashboard shows a forked pipeline: "已接收" → [LLM branch: 排队→翻译中→已完成] | [TM branch: 精确·模糊·模式→已命中]; the tree-fork layout is the intentional design (visually shows data flow splitting) — do NOT replace with flat lane-card layout; term extraction is a sub-process of the LLM branch (`.branch-sub`), not a standalone section; LLM completed count is computed as `totalTranslated - (tmHits + tmFuzzyHits + tmPatternHits)`; TM branch uses `--secondary` color, real-time `TranslationStats` fields (not separately-fetched `TranslationMemoryStats`)
