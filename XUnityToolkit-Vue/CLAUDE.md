...
- **Gradient on `<button>`：** Custom-styled buttons need `appearance: none`; gradient backgrounds (`conic-gradient`, etc.) with `border: Npx solid transparent` need `background: <gradient> border-box` — default `padding-box` origin misaligns the gradient at rounded edges
