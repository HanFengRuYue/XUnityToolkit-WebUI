...
```bash
npm run dev          # 开发服务器（API 代理到 http://127.0.0.1:51821）
npm run build        # 构建到 ../XUnityToolkit-WebUI/wwwroot/
npx vue-tsc --build  # 类型检查
```

## 目录结构

- `src/views/` — 页面组件（顶层 + 游戏子页面）
- `src/api/` — API 客户端（`client.ts` axios 封装、`types.ts` 共享类型、`games.ts` 等）
- `src/stores/` — Pinia stores（`games`、`sidebar`、`theme`、`install`、`update`）
- `src/composables/` — 组合式函数（`useAutoSave`、`useFileExplorer`、`useAddGameFlow`、`useWindowControls`）
- `src/constants/` — 共享常量（`prompts.ts` 默认系统提示词）
- `src/utils/` — 工具函数（`format.ts` formatBytes/formatSpeed）
...
- **ConfigPanel 后端 INI 修改同步：** 当后端操作在 ConfigPanel 之外修改 INI（如 TMP 字体安装写入 `FallbackFontTextMeshPro`、字体生成安装），ConfigPanel 必须用 `disableAutoSave() → getConfig() → form.value = { ...config } → nextTick() → enableAutoSave()` 刷新——否则 2 秒自动保存会用旧 form 值覆盖后端写入
