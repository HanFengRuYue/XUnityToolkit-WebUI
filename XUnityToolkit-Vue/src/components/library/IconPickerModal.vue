...
const iconImages = ref<SteamGridDbImage[]>([])
...
async function loadIcons(steamGridDbGameId: number) {
...
async function selectIconImage(image: SteamGridDbImage) {
...
<template>
  ...
  >
    <NTabs type="segment" animated>
      <NTabPane name="steamgriddb" tab="SteamGridDB">
        <div class="search-section">
          ...
          <div v-if="selectedGameId" class="icons-section">
            <div class="icons-header">
              <NButton size="tiny" quaternary @click="selectedGameId = null; iconImages = []">
                ← 返回搜索结果
              </NButton>
            </div>
            <div v-if="loadingIcons" class="center-state">
              <NSpin size="small" />
            </div>
            <div v-else-if="iconImages.length === 0" class="center-state">
              <span class="muted-text">未找到图标</span>
            </div>
            <div v-else class="icons-grid">
              <div
                v-for="image in iconImages"
                :key="image.id"
                class="icon-image-item"
                :class="{ saving: savingIcon }"
                @click="selectIconImage(image)"
              >
                <img :src="image.thumb" :alt="`Icon ${image.id}`" loading="lazy" />
          ...
          <div v-if="!searching && !selectedGameId && searchResults.length === 0 && searchQuery.trim()" class="center-state">
            <span class="muted-text">按 Enter 搜索 SteamGridDB 图标</span>
          </div>
        </div>
      </NTabPane>

      <NTabPane name="web" tab="网络搜索">
        <WebImageSearchTab :game="game" mode="icon" @saved="onWebSaved" />
      </NTabPane>
    ...
    <template #footer>
      <div class="modal-footer">
        <NButton size="small" type="error" quaternary @click="deleteIcon">删除自定义图标</NButton>
      </div>
    </template>
  </NModal>
</template>
...
.icon-image-item:hover .icon-image-overlay {
  opacity: 1;
}
